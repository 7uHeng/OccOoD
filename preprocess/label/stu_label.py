...
def main(config):
    ...
    for sequence in sequences:
        ...
        # )
        sequence_path = os.path.join(
            config.kitti_root, sequence
        )
        label_paths = sorted(
            glob.glob(os.path.join(sequence_path, "*.label"))
        )
        invalid_paths = sorted(
            glob.glob(os.path.join(sequence_path, "*.invalid"))
        )
        ...
        for i in tqdm(range(len(label_paths))):
            ...
            LABEL = remap_lut[LABEL.astype(np.uint16)].astype(
                np.float32
            )  # Remap 21 classes semanticKITTI SSC
            LABEL[
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("./stu_label.py")
    parser.add_argument(
        '--kitti_root',
        '-r',
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help='vaa-kitti_preprocess_root',
    )
