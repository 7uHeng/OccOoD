...
class stu_Dataset(Dataset):
    def __init__(
        ...
        self.class_names =  [ "empty", "car", "bicycle", "motorcycle", "truck", 
                            ...
                            "vegetation", "trunk", "terrain", "pole", "traffic-sign"]#plus anomaly class
    ...
    def load_poses(self):
        ...
        for sequence in self.sequences:
            pose_path = os.path.join(self.data_root, sequence, "poses.txt")
            calib = self.read_calib(
                os.path.join(self.data_root, sequence, "calib.txt")
            )
    ...
    def load_scans(self):
        ...
        for sequence in self.sequences:
            ...
            P_scaled = P.copy()
            P_scaled[0, 0] *= scale_w  # fx
            P_scaled[0, 2] *= scale_w  # cx
            P_scaled[1, 1] *= scale_h  # fy
            P_scaled[1, 2] *= scale_h  # cy            
            
            T_velo_2_cam = calib["Tr"]
#             proj_matrix = P @ T_velo_2_cam
            proj_matrix = P_scaled @ T_velo_2_cam

            glob_path = os.path.join(
                self.data_root, sequence, "voxels", "*.label"
            )
    ...
    def get_data_info(self, index):
        ...
        meta_dict = self.get_meta_info(scan, sequence, frame_id) # only one frame
    ...
    def get_meta_info(self, scan, sequence, frame_id):
        ...
        pts_filename = os.path.join(
                self.data_root, f"sequences_{self.depthmodel}_lidar/", sequence, frame_id + ".bin"
            )
        ...
        for i in self.target_frames:#[],none
            id = int(frame_id)
    ...
    def get_input_info(self, sequence, frame_id):
        ...
        # reference frame
        for i in self.target_frames:#[],one frame
            id = int(frame_id)
