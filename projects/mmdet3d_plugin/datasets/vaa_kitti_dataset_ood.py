...
class VAA_Kitti_OoD_Dataset(Dataset):
    def __init__(
        self,
        split,
        test_mode,
    ...
    def load_scans(self):
        ...
        for sequence in self.sequences:
            ...
            glob_path = os.path.join(
                self.data_root, "dataset", "sequences", sequence, "voxels", "*.label"
            )
    ...
    def get_meta_info(self, scan, sequence, frame_id):
        ...
        pts_filename = os.path.join(
                self.data_root, f"sequences_{self.depthmodel}_lidar/sequences", sequence, frame_id + ".bin"
            )
