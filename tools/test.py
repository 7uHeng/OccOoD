...
from projects.mmdet3d_plugin.occood.apis.test import custom_multi_gpu_test, custom_single_gpu_test
...
def evaluate_ood(anomaly_score, ood_gts, verbose=True):
    ...
    auroc, aupr, fpr = calculate_ood_metrics(np.concatenate([ind_out, ood_out]), 
                                              np.concatenate([ind_label, ood_label]))
...
def main():
    ...
    if not distributed:
        ...
        # outputs, anomaly_scores, ood_gts = custom_single_gpu_test(model, data_loader, args.show, args.show_dir)
    else:
        ...
        outputs = custom_multi_gpu_test(model, data_loader, args.tmpdir,
                                        args.gpu_collect)
        # outputs, anomaly_scores, ood_gts = custom_multi_gpu_test(model, data_loader, args.tmpdir,
        #                                 args.gpu_collect)        
